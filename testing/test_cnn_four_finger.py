...
def test_cnn_four_finger(data_type, model_type):
    ...
    if data_type == 'open_close_test':
        test_x = np.load(os.path.join(curr_path, '../data_dependencies/labeled_data/fist_relax/four_fingers/us_test.npy')).flatten()
    ...
    # model = None
    if model_type == 'fr':
        model = tf.keras.models.load_model(os.path.join(curr_path, './models/four_fingers/fist_relax_convolutional.h5'))
    elif model_type == 'pr':
        model = tf.keras.models.load_model(os.path.join(curr_path, './models/four_fingers/pinch_relax_convolutional.h5'))
    ...
    errors = predict(preds, test_y, os.path.join(curr_path, '../static/four_fingers'))
